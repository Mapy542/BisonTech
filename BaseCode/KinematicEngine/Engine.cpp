...
void Destination(double x, double y, double r, double speed) {
  ...
  if (0.15 < FL.velocity(percent) ||
      (0.15 < RL.velocity(percent) ||
       (0.15 < FR.velocity(percent) || 0.15 < RR.velocity(percent)))) {
    RampUp = 1.0;
  }
  ...
  while (!((fabs((y - CurrentYAxis)) < 10.0 && fabs((TangentialDist)) < 10.0) &&
    ...
    RampDown = ((fabs((x - CurrentXAxis)) + fabs((y - CurrentYAxis))) +
                1.5 * fabs((TangentialDist))) *
               -0.015;
    RampDown = (RampDown * RampDown + 15.0) /
               100.0; // 15 defines the floor or minimum value for ramp down. At
                      // min, 15% of max speed.
    if (1.0 < RampDown) {
      RampDown = 1.0;
    }
    ...
    if (fabs((x - CurrentXAxis)) < fabs((TangentialDist)) &&
        fabs((y - CurrentYAxis)) < fabs((TangentialDist))) {
      ...
      if (fabs((x - CurrentXAxis)) < fabs((y - CurrentYAxis))) {
        DriveYScale = 100.0;
        DriveXScale =
            100.0 * (fabs((x - CurrentXAxis)) / fabs((y - CurrentYAxis)));
        DriveRScale =
            50.0 * (fabs((TangentialDist)) / fabs((y - CurrentYAxis)));
      } else {
        DriveYScale =
            100.0 * (fabs((y - CurrentYAxis)) / fabs((x - CurrentXAxis)));
        DriveXScale = 100.0;
        DriveRScale =
            50.0 * (fabs((TangentialDist)) / fabs((x - CurrentXAxis)));
      }
    }
    ...
    DriveXPower =
        (DriveYDir * sin(Gyroscope.heading(degrees) * M_PI / 180) +
         (DriveXDir * cos(Gyroscope.heading(degrees) * M_PI / 180)) * -1.0) *
        1.0;
    DriveYPower = (DriveYDir * cos(Gyroscope.heading(degrees) * M_PI / 180) +
                   DriveXDir * sin(Gyroscope.heading(degrees) * M_PI / 180)) *
                  1.0;
    // Apply motor powers to individual motors
    DriveMotors(DriveXPower, DriveYPower, DriveRPower,
                speed * (RampUp * RampDown));
    // Print_XYR();
...
void XArc(double x, double y, double r, double speed, bool drivetype) {
  ...
  while (!(fabs((y - CurrentYAxis)) < (10.0 + increasediffs) &&
           fabs((TangentialDist)) < (10.0 + increasediffs) &&
           fabs((x - CurrentXAxis)) < 10.0 + (increasediffs))) {
    ...
    if (fabs((x - CurrentXAxis)) < fabs((TangentialDist)) &&
      ...
      if (fabs((x - CurrentXAxis)) < fabs((y - CurrentYAxis))) {
        ...
        DriveYScale = xCompleationRatio *
                      (fabs((y - CurrentYAxis)) / fabs((x - CurrentXAxis)));
        DriveXScale = 100.0;
        DriveRScale =
            50.0 * (fabs((TangentialDist)) / fabs((x - CurrentXAxis)));
      }
  ...
  while (!(fabs((y - CurrentYAxis)) < (10.0 + increasediffs) &&
    ...
    if (fabs((x - CurrentXAxis)) < fabs((TangentialDist)) &&
      ...
      DriveXScale = YCompleationRatio *
                    (fabs((x - CurrentXAxis)) / fabs((TangentialDist)));
      DriveRScale = 50.0;
    } else {
      if (fabs((x - CurrentXAxis)) < fabs((y - CurrentYAxis))) {
        DriveYScale = 100.0;
        DriveXScale = YCompleationRatio *
                      (fabs((x - CurrentXAxis)) / fabs((y - CurrentYAxis)));
        DriveRScale =
            50.0 * (fabs((TangentialDist)) / fabs((y - CurrentYAxis)));
      } else {
        DriveYScale =
