...
void EncoderIntegral()
{                               // Update odometry from encoders by integrating encoder values
  ...
  if (abs(DeltaTheta) > 300)
  {                                           // If heading wraps around 360 degrees, then guess theta based on current speed
